...
/* Default MQTT broker; overridable at runtime via the "mqtt_config"/"broker_url"
   NVS entry (see load_mqtt_config()). */
#define MQTT_BROKER_URI_DEFAULT "mqtt://test.mosquitto.org:1883"
#define MQTT_BROKER_URI_MAX 128
#define REGISTRY_TOPIC "devices/registry"

/* OTA notifications: {"version": "...", "url": "...", "sha256": "..."} */
#define OTA_TOPIC "devices/ota"
...
/* MQTT broker configuration (NVS), ready for future broker migration. */
...
/* MQTT broker URI, loaded from NVS on boot (falls back to the default). */
static char mqtt_broker_uri[MQTT_BROKER_URI_MAX] = MQTT_BROKER_URI_DEFAULT;

/* Guards against overlapping OTA downloads (retained notifications can be
...
/* ---------- Persisted MQTT broker configuration ---------- */

/* Loads the broker URI from NVS, keeping the compile-time default when the
   namespace/key does not exist yet (first boot). */
static void load_mqtt_config(void)
{
    nvs_handle_t handle;
    if (nvs_open(MQTT_CONFIG_NAMESPACE, NVS_READONLY, &handle) == ESP_OK)
    {
        size_t len = sizeof(mqtt_broker_uri);
        if (nvs_get_str(handle, NVS_KEY_BROKER_URL, mqtt_broker_uri, &len) != ESP_OK ||
            strlen(mqtt_broker_uri) == 0)
        {
            snprintf(mqtt_broker_uri, sizeof(mqtt_broker_uri), "%s", MQTT_BROKER_URI_DEFAULT);
        }
        nvs_close(handle);
    }
...
/* Persists a new broker URI. It takes effect on the next MQTT start (boot). */
static bool save_broker_url(const char *url)
{
    if (url == NULL || strlen(url) == 0 || strlen(url) >= MQTT_BROKER_URI_MAX)
    {
        return false;
    }

    nvs_handle_t handle;
    if (nvs_open(MQTT_CONFIG_NAMESPACE, NVS_READWRITE, &handle) != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to open NVS to save broker URL");
        return false;
    }
    esp_err_t err = nvs_set_str(handle, NVS_KEY_BROKER_URL, url);
    if (err == ESP_OK)
    {
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to save broker URL: %s", esp_err_to_name(err));
        return false;
    }

    snprintf(mqtt_broker_uri, sizeof(mqtt_broker_uri), "%s", url);
    ESP_LOGI(TAG, "Saved MQTT broker URL to NVS: %s", mqtt_broker_uri);
    return true;
}
...
/* Parsed semantic version: dotted numeric release + optional pre-release. */
typedef struct
{
    long num[4];
    int nnum;
    const char *pre; /* pre-release suffix after '-', or NULL */
} semver_t;

static void parse_version(const char *s, semver_t *v)
{
    memset(v, 0, sizeof(*v));
    const char *p = s;
    while (v->nnum < (int)(sizeof(v->num) / sizeof(v->num[0])) && *p >= '0' && *p <= '9')
    {
        char *end = NULL;
        v->num[v->nnum++] = strtol(p, &end, 10);
        p = end;
        if (*p == '.')
        {
            p++;
...
/* Compares two pre-release strings, treating embedded digit runs numerically
   so that e.g. "rc10" sorts after "rc2". */
...
/* Returns <0 if a<b, 0 if equal, >0 if a>b. A pre-release is older than the
   matching final release (1.0.0 > 1.0.0-rc2 > 1.0.0-rc1). */
static int version_compare(const char *a, const char *b)
{
    ...
    int count = va.nnum > vb.nnum ? va.nnum : vb.nnum;
    for (int i = 0; i < count; i++)
    {
        long x = i < va.nnum ? va.num[i] : 0;
        long y = i < vb.nnum ? vb.num[i] : 0;
        if (x != y)
        {
            return x < y ? -1 : 1;
        }
    ...
    return compare_prerelease(va.pre, vb.pre);
}
...
/* Computes the SHA-256 of the first `length` bytes of the (already written)
   OTA partition and compares it with the expected hex digest. */
static bool verify_partition_sha256(const esp_partition_t *partition, size_t length,
                                    const char *expected_hex)
{
    uint8_t expected[32];
    if (!hex_to_bytes(expected_hex, expected, sizeof(expected)))
    {
        ESP_LOGE(TAG, "Malformed SHA-256 in OTA notification");
        return false;
    }
    ...
    while (offset < length)
    {
        ...
        if (err != ESP_OK)
        {
            ESP_LOGE(TAG, "Failed to read OTA partition for verification: %s", esp_err_to_name(err));
            psa_hash_abort(&op);
            return false;
        }
        ...
        if (status != PSA_SUCCESS)
        {
            ESP_LOGE(TAG, "psa_hash_update failed: %d", (int)status);
            psa_hash_abort(&op);
            return false;
        }
        offset += chunk;
    ...
    uint8_t actual[32];
    size_t actual_len = 0;
    status = psa_hash_finish(&op, actual, sizeof(actual), &actual_len);
    if (status != PSA_SUCCESS || actual_len != sizeof(actual))
    {
...
/* Downloads the image over HTTPS, writes it to the passive OTA slot, verifies
   the SHA-256 against the notification and only then marks it bootable.
   esp_https_ota has no hook for hashing the raw stream, so the written image
   is read back and hashed before esp_https_ota_finish() switches the boot
   partition; a mismatch aborts the update, leaving the current image intact. */
static void perform_ota_update(const char *url, const char *expected_sha256)
{
    ...
    esp_http_client_config_t http_config = {
        ...
        /* GitHub release assets redirect to a signed objects.githubusercontent.com
           URL. Its headers exceed the 512-byte default RX buffer and its
           path+query exceed the default TX buffer, which makes the client fail
           with "Out of buffer". */
        .buffer_size = 4096,
        .buffer_size_tx = 4096,
    };
    ...
    while (1)
    {
        ...
        if (written - last_logged >= 262144) /* ~256 KiB */
        {
    ...
    if (image_len <= 0)
    {
        ESP_LOGE(TAG, "OTA image length invalid (%d)", image_len);
        esp_https_ota_abort(ota_handle);
        return;
    }

    ESP_LOGI(TAG, "Download complete (%d bytes), verifying SHA-256...", image_len);
    if (!verify_partition_sha256(update_partition, (size_t)image_len, expected_sha256))
    {
        ESP_LOGE(TAG, "SHA-256 mismatch - aborting OTA, image discarded");
        esp_https_ota_abort(ota_handle);
        return;
    }
...
static void ota_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "OTA task started (target version %s)", req->version);

    perform_ota_update(req->url, req->sha256);

    /* Reached only if the update failed: allow a later notification to retry. */
    ESP_LOGW(TAG, "OTA task finished without a reboot");
...
static void handle_ota_notification(const char *data, int len)
{
    cJSON *json = cJSON_ParseWithLength(data, len);
    if (json == NULL)
    {
        ESP_LOGW(TAG, "Ignoring malformed OTA notification");
        return;
    }

    const cJSON *jversion = cJSON_GetObjectItemCaseSensitive(json, "version");
    const cJSON *jurl = cJSON_GetObjectItemCaseSensitive(json, "url");
    const cJSON *jsha256 = cJSON_GetObjectItemCaseSensitive(json, "sha256");
    if (!cJSON_IsString(jversion) || !cJSON_IsString(jurl) || !cJSON_IsString(jsha256))
    {
        ESP_LOGW(TAG, "OTA notification missing version/url/sha256");
        cJSON_Delete(json);
        return;
    }

    const char *version = jversion->valuestring;
    const char *url = jurl->valuestring;
    const char *sha256 = jsha256->valuestring;

    ESP_LOGI(TAG, "OTA notification: version=%s url=%s", version, url);

    if (version_compare(version, FIRMWARE_VERSION) <= 0)
    {
        ESP_LOGI(TAG, "Running %s; OTA version %s is not newer, skipping",
    ...
    if (ota_in_progress)
    {
        ESP_LOGW(TAG, "OTA already in progress, ignoring notification for %s", version);
        cJSON_Delete(json);
        return;
    }
    ...
    {
        req->url = strdup(url);
        req->sha256 = strdup(sha256);
        snprintf(req->version, sizeof(req->version), "%s", version);
    }

    if (req == NULL || req->url == NULL || req->sha256 == NULL)
    {
    ...
    if (xTaskCreate(ota_task, "ota_update", OTA_TASK_STACK, req, 5, NULL) != pdPASS)
    {
...
/* Cancels a pending rollback once the freshly booted image has proven it can
   reach the network and the broker. */
static void ota_mark_valid_if_pending(void)
{
    ...
    if (esp_ota_get_state_partition(running, &ota_state) == ESP_OK)
    {
        ...
        {
            ESP_LOGI(TAG, "New firmware booted successfully, marking valid (rollback cancelled)");
                               ...
                               int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        /* The boot-time OTA check relies on the retained notification being
           delivered right after this subscription. */
        ESP_LOGI(TAG, "Checking for OTA updates on boot...");

        /* We are online with the broker: the running image is good. */
    ...
    case MQTT_EVENT_DATA:
            ...
            strncmp(event->topic, OTA_TOPIC, event->topic_len) == 0)
        {
            ...
            {
                handle_ota_notification(event->data, event->data_len);
            }
            else
            {
                ESP_LOGW(TAG, "Ignoring chunked OTA message (%d/%d bytes)",
                         event->data_len, event->total_data_len);
            }
        }
        else if (strncmp(event->topic, topic_led_set, event->topic_len) == 0)
        {
...
static void ble_rx_line_cb(const char *line, size_t len)
{
    ...
    else if (strcmp(type, "set_broker") == 0)
    {
        char url[MQTT_BROKER_URI_MAX];
        if (json_get_string(line, "url", url, sizeof(url)) && save_broker_url(url))
        {
            ble_prov_send_status("ok", "broker url saved (applies after reboot)");
        }
        else
        {
            ble_prov_send_status("error", "invalid broker url");
        }
    }
    else
    {
